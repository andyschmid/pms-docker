...
RUN apt-get update && \
    ...
    libavformat-dev libavcodec-dev mkvtoolnix vim && \
...
ADD ./process.sh /opt/PlexComskip/process.sh

RUN chmod 755 /opt/PlexComskip/comskip.sh
RUN chmod 755 /opt/PlexComskip/process.sh
